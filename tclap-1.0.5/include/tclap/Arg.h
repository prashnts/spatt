...
class Arg
{
	private: 
		...
		static bool& allowDashRef() { static bool allowDash = false ; return allowDash; }
	...
	public:
		...
		static bool allowDash() { return allowDashRef(); }

		static void setAllowDash(bool b) { allowDashRef() = b; }
  ...
  _xorSet(false)
{
	...
	if (Arg::allowDash() == false && 
	    (_name.find( Arg::flagStartString(), 0 ) != std::string::npos  || 
	...
	if ( _name.find( " ", 0 ) != std::string::npos ) 
		throw(SpecificationException("Argument name cannot contain space.",
							toString() ) );
